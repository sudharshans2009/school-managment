...
export default function TeacherHomeworkPage() {
  ...
  const { data: homework } = useQuery<Homework[]>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/homework?classroomId=${selectedClassroom}&teacherId=${session?.user?.id}`);
  ...
  const { data: submissions, isLoading: submissionsLoading } = useQuery<Submission[]>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/homework/submissions?homeworkId=${selectedHomework}`);
  ...
  const gradeMutation = useMutation({
    mutationFn: async (data: { id: string; marksObtained: number; feedback: string }) => {
            ...
            {selectedClassroom && (
              <>
                ...
                <div>
                  ...
                  <Select value={viewMode} onValueChange={(v) => setViewMode(v as "homework" | "students")}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
        ...
        <Card>
          ...
          <CardContent>
            ...
            {students && students.length > 0 && (
              <div className="mt-6">
                <h3 className="font-semibold mb-4">Mark Physical Submissions</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {students
                    .filter((student) => !submittedStudentIds.includes(student.id))
                    .map((student) => (
                      <Card key={student.id}>
      ...
      <Dialog open={openMarkDialog} onOpenChange={setOpenMarkDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <p>
              Confirm that <strong>{selectedStudent?.user.name}</strong> has submitted the
              homework physically?
            </p>
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setOpenMarkDialog(false)}>
                Cancel
              </Button>
              <Button
      ...
      <Dialog open={openGradeDialog} onOpenChange={setOpenGradeDialog}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmitGrade} className="space-y-4">
            ...
            <div>
              <Label htmlFor="marks">Marks Obtained (out of {selectedSubmission?.totalMarks})</Label>
              <Input
                id="marks"
                name="marks"
                type="number"
