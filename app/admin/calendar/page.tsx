...
import { Calendar, ChevronLeft, ChevronRight, Edit2, Trash2, Sun, Clock, Users, Briefcase, Building2 } from "lucide-react";
...
export default function AdminCalendarPage() {
  ...
  useQuery({
    queryKey: ["calendar-day", selectedDate ? format(selectedDate, "yyyy-MM-dd") : ""],
    queryFn: async () => {
      if (!selectedDate) return null;
      const res = await fetch(`/api/calendar?date=${format(selectedDate, "yyyy-MM-dd")}`);
  ...
  const getDayBadge = (date: Date) => {
    ...
    if (isDefault) {
      if (dayOfWeek === 0) {
        return <Badge variant="outline" className="text-xs">Sunday</Badge>;
      }
      return <Badge variant="secondary" className="text-xs">Working</Badge>;
    }

    // Custom configuration
    if (config.dayType === "holiday") {
      const icon = config.holidayFor === "students" ? <Users className="w-3 h-3" /> :
                    config.holidayFor === "teachers" ? <Briefcase className="w-3 h-3" /> :
                    config.holidayFor === "office" ? <Building2 className="w-3 h-3" /> :
    ...
    if (config.customTimetable) {
      return <Badge variant="secondary" className="text-xs">
        {DAYS_OF_WEEK[config.customTimetable === 6 ? 6 : config.customTimetable]} TT
      </Badge>;
    }
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Configure {selectedDate && format(selectedDate, "EEEE, MMMM d, yyyy")}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFormData({ ...formData, dayType: value as "working" | "holiday" })
                }
              >
                <SelectTrigger>
                  <SelectValue />
            ...
            {formData.dayType === "working" && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setFormData({ ...formData, dayDuration: value as "full" | "half" })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setFormData({
                        ...formData,
                        holidayFor: value as "all" | "students" | "teachers" | "office",
                      })
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      <SelectItem value="all">All (Students, Teachers, Office Staff)</SelectItem>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setFormData({
                      ...formData,
                      customTimetable: value === "default" ? null : parseInt(value),
                    })
                  }
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="default">Default (Based on actual day)</SelectItem>
                ...
                <p className="text-xs text-muted-foreground">
                  Choose a different day&apos;s timetable to follow (e.g., use Monday&apos;s schedule on Saturday)
                </p>
              </div>
            )}
          ...
          <div className="flex justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleSave} disabled={saveMutation.isPending}>
                {saveMutation.isPending && <Spinner className="w-4 h-4 mr-2" />}
                Save Configuration
              </Button>
            </div>
          </div>
